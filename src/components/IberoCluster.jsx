import React, { useRef } from 'react';
import { Intro, InfoHD1, InfoHD2, InfoHD3, InfoHD4, LogoHD, iberoReporte, iberoDash1, iberoDash2, iberoClusterDetalle } from "../assets";
import { motion, useInView } from "framer-motion";

function MUCD() {
    const sectionRefs = Array.from({ length: 5 }, () => useRef(null)); // Crear un arreglo con 4 referencias

    const isInView = sectionRefs.map((ref, index) =>
        useInView(ref, {
            once: true, // Ejecutar la animación solo una vez
            threshold: index === 1 ? 0.1 : 0.5 // Umbral específico para index === 1
        })
    );

    return (
        <div className="bg-white w-full mt-24">
            {/* Contenedor principal centrado */}
            <div ref={sectionRefs[0]} className="div">
                <div className="container mx-auto p-4 max-w-screen-xl">
                    {/* Contenedor para título y párrafo */}
                    <div className="grid grid-cols-1 md:grid-cols-2 md:gap-10 h-[300px] md:h-[500px]">
                        {/* Contenedor del título */}
                        <motion.div
                            className="flex text-center md:text-start justify-center md:justify-start items-center mb-5"
                            initial={{ opacity: 0, y: 50 }}
                            animate={isInView[0] ? { opacity: 1, y: 0 } : {}}
                            transition={{ delay: 0.3, duration: 1 }}
                        >
                            <h1 className="text-[40px] md:text-[60px] font-semibold font-montserrat leading-none">
                                Benchmark <br /> Ibero
                            </h1>
                        </motion.div>

                        {/* Contenedor del párrafo */}
                        <motion.div
                            className="flex text-center md:text-start items-center"
                            initial={{ opacity: 0, y: 50 }}
                            animate={isInView[0] ? { opacity: 1, y: 0 } : {}}
                            transition={{ delay: 0.3, duration: 1 }}
                        >
                            <p className=" font-notosans font-light leading-relaxed md:text-lg text-textlight">
                                El proyecto consistió en mapear la oferta educativa de instituciones de nivel superior con programas académicos similares a los de la Universidad Ibero. El objetivo fue comparar costos y duración de los programas, proporcionando información clave para la toma de decisiones estratégicas de la universidad. 


                            </p>
                        </motion.div>
                    </div>
                </div>

                {/* Contenedor de imágenes (ocupa todo el ancho de la pantalla) */}
                <motion.div
                    className="grid grid-cols-1 w-full mt-10 mx-auto"
                    initial={{ clipPath: 'inset(0% 0% 0% 100%)' }}
                    animate={isInView[0] ? { clipPath: 'inset(0% 0% 0% 0%)' } : {}}
                    transition={{ delay: 0.5, duration: 0.4 }}
                >
                    {/* Imagen 1 */}
                    <div className="bg-iberoBlue flex items-center justify-center px-16 py-10 ">
                        <motion.div className="div"
                            initial={{ opacity: 0, y: 50 }}
                            animate={isInView[0] ? { opacity: 1, y: 0 } : {}}
                            transition={{ delay: 0.9, duration: 1 }}
                            style={{ position: 'relative', zIndex: 1 }}>
                            <img
                                src={iberoReporte}
                                alt="Imagen 1"
                                className="max-w-screen-lg w-full" />
                        </motion.div>
                    </div>
                </motion.div>
            </div>

            <div className="div">
                {/* Contenedor de imágenes (ocupa todo el ancho de la pantalla) */}
                <div className="grid grid-cols-1 xl:grid-cols-2 w-full "  >
                    {/* Imagen 3 */}
                    <div className="bg-iberoLightBlue flex items-center justify-center px-20 lg:py-14 xl:py-30">
                        <motion.div className="div"
                            ref={sectionRefs[1]}
                            initial={{ opacity: 0, y: 50 }}
                            animate={isInView[1] ? { opacity: 1, y: 0 } : {}}
                            transition={{ delay: 0.3, duration: 1 }}>
                            <img
                                src={iberoDash1}
                                alt="Imagen 3"
                                className="w-full"
                            />
                        </motion.div>
                    </div>

                    {/* Imagen 4 */}
                    <div className="bg-iberoLightBlue flex items-center justify-center px-20 lg:py-14 xl:py-30">
                        <motion.div className="div"
                            ref={sectionRefs[1]}
                            initial={{ opacity: 0, y: 50 }}
                            animate={isInView[1] ? { opacity: 1, y: 0 } : {}}
                            transition={{ delay: 0.3, duration: 1 }}>
                            <img
                                src={iberoDash2}
                                alt="Imagen 4"
                                className="w-full"
                            />
                        </motion.div>
                    </div>
                </div>
            </div>

            

                    <motion.div
                        ref={sectionRefs[2]}
                        className="flex text-center md:text-start items-center max-w-screen-md mx-auto min-h-[400px] justify-center"
                        initial={{ opacity: 0, y: 50 }}
                        animate={isInView[2] ? { opacity: 1, y: 0 } : {}}
                        transition={{ delay: 0.3, duration: 1 }}
                    >
                        <p className=" font-notosans font-light leading-relaxed md:text-lg text-textlight px-10">
                        Participé, además del diseño del reporte, en la creación de un tablero interactivo que mostrara los resultados a detalle y propuse una visualización extra utilizando un algoritmo llamado K-means que facilitara la ubicación de la Ibero frente a las demás universidades.
                        </p>
                    </motion.div>

               

            <div className="grid grid-cols-1 w-full">

                {/* Imagen 5 */}
                <div className="bg-iberoLightGray flex items-center justify-center px-10 py-10">
                    <motion.div
                        ref={sectionRefs[3]}
                        className="div"
                        initial={{ opacity: 0, y: 50 }}
                        animate={isInView[3] ? { opacity: 1, y: 0 } : {}}
                        transition={{ delay: 0.3, duration: 1 }}
                    >
                        <img src={iberoClusterDetalle} alt="Imagen 3" className="max-w-[1080px]"/>

                        <p className="font-notosans font-light leading-relaxed md:text-lg text-enfoqueBlue items-center text-center text-xl mt-4">
                            Para ver el tablero haz{" "}
                            <a
                                href="https://abrajam83.github.io/ibero/"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-enfoqueBlue font-bold underline hover:text-iberoBlue transition"
                            >
                                click aquí
                            </a>
                        </p>
                    </motion.div>
                </div>
            </div>
        </div>
    );
}

export default MUCD;
